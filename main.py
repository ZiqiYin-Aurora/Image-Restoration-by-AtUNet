...
print(dir_name)
# log_dir = os.path.join(dir_name, 'log', opt.arch + '_logs')
# log_dir = os.path.join(dir_name, 'log_CBSD', opt.arch + '_logs')
# log_dir = os.path.join(dir_name, 'log_pro_4', opt.arch + '_logs') # model -- del FFN
# log_dir = os.path.join(dir_name, 'log_deblur', opt.arch + '_logs')
log_dir = os.path.join(dir_name, 'log_deblur', opt.arch + '_logs')
...
if opt.arch == 'DnCNN_Pro':
    model = AaCNN(channels=channel, num_of_layers=layers_num)
...
# model.load_state_dict(torch.load(opt.pretrain_weights))
# print(model)
...
if warmup:
    ...
    # scheduler_cyclic = torch.optim.lr_scheduler.CyclicLR(optimizer, 1e-6, max_lr, step_size_up=500, step_size_down=500,
    #                                               mode='triangular', gamma=1.0, scale_fn=None, scale_mode='cycle',
    #                                               cycle_momentum=False, base_momentum=0.8, max_momentum=0.9)
    # scheduler = GradualWarmupScheduler(optimizer, multiplier=1, total_epoch=warmup_epochs,
    #                                    after_scheduler=scheduler_cyclic)
    scheduler.step()
else:
    scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr, max_lr, step_size_up=500, step_size_down=500,
                                                  mode='triangular', gamma=1.0, scale_fn=None, scale_mode='cycle',
...
for epoch in range(start_epoch, opt.epoch + 1):
    ...
    for i, imdata in enumerate(train_loader, 0):
        ...
        # # make noise data
        # if mode == 'S':
        #     noise = torch.FloatTensor(imdata.size()).normal_(mean=0, std=noise_level / 255.)
        # if mode == 'B':
        #     noise = torch.zeros(imdata.size())
        ...
        # noisy_img = imdata + noise
        # noisy_img = Variable(noisy_img.cuda())
        # ground_truth = Variable(imdata.cuda())
        ...
        with torch.cuda.amp.autocast():
            ...
            if opt.arch == 'DnCNN_Pro':
                denoised_img = torch.clamp(noisy_img-train_out, 0., 1.)
            else:
                # print(train_out.size())
                # loss = criterion(train_out, noise) / (noisy_img.size()[0]*2)  # DnCNN
        ...
        print("[epoch %d/%d][%d/%d] loss: %.4f PSNR_train: %.4f" % (epoch, opt.epoch, i + 1, len(train_loader), loss.item(), psnr_train.item()))
            ...
            with torch.no_grad():
                ...
                for j, valdata in enumerate(val_loader, 0):
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if opt.arch == 'DnCNN_Pro':
                            denoised_val_img = torch.clamp(noisy_val_img-val_out, 0., 1.)
                        else:
                            denoised_val_img = torch.clamp(val_out, 0., 1.)
                    psnr_sum += batch_PSNR(ground_truth, denoised_val_img, True).item()
                ...
                if psnr > best_psnr:
                    ...
                    torch.save({'epoch': epoch,
                                ...
                                }, os.path.join(model_dir, "model_best_GoPro.pth"))
                    ...
                    # torch.save(model.state_dict(), os.path.join(save_fp, 'net_paras_deblur.pth'))
    ...
    with torch.no_grad():
        ...
        for i, valdata in enumerate(val_loader, 0):
            ...
            # denoised_val_img = torch.clamp(val_out, 0., 1.)
            sum_of_psnr += batch_PSNR(ground_truth, denoised_val_img, True).item()
            sum_of_ssim += batch_SSIM(ground_truth, denoised_val_img, True).item()
            index += 1

    avg_psnr = sum_of_psnr / (index)
